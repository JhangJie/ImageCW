...
LabelArr Minst::read_Lable(const char* filename)// 读入图像
{
	FILE* fp = NULL;
	fp = fopen(filename, "rb");
	if (fp == NULL)
...
char* Minst::intTochar(int i)// 将数字转换成字符串
{
	int itemp = i;
	int w = 0;
	while (itemp >= 10) 
...
char* Minst::combine_strings(char* a, char* b) // 将两个字符串相连
{
	char* ptr;
...
void Minst::save_Img(ImgArr imgarr, char* filedir) // 将图像数据保存成文件
{
