...
int AT_(float* mat, int x, int y)	//待修正
{
	return mat[x * y];
}

void AT(float* mat, int x, int y, int data)	//待修正
{
	mat[x * y] = data;
}
...
void Imagematting::loadImage(unsigned char* inptr, int w, int h)
{
	...
	matte = new unsigned char[w * h * 3];	//注意
...
void Imagematting::loadTrimap(unsigned char* inptr, int w, int h)
{
	...
	allsize = 0;
	for (int n = 0; n < h; n++)	//計數Pixel為0 Count ( 二值化data )
	{
		for (int m = 0; m < w; m++)
		{
			//printf("dilateimg = %d\n",dilateimg[n * w * 3 + 3 * m]);
